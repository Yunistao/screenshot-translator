...
type RenderSelectionExportOptions = {
    ...
    switch (annotation.type) {
      ...
      case 'arrow':
        if (
          ...
          drawArrow(
            ctx,
            annotation.startX - selectionArea.x,
            annotation.startY - selectionArea.y,
            annotation.endX - selectionArea.x,
            annotation.endY - selectionArea.y,
          );
        }
        break;
      case 'brush':
        if (annotation.points && annotation.points.length > 1) {
          ctx.beginPath();
          ctx.moveTo(
            annotation.points[0].x - selectionArea.x,
            annotation.points[0].y - selectionArea.y,
          );
...
export const renderSelectionExport = async ({
  screenshotImage,
  selectionArea,
  annotations = [],
  ocrLines = [],
  includeTranslation = false,
}: RenderSelectionExportOptions): Promise<string> => {
...
export const renderSelectionExportBlob = async (
  options: RenderSelectionExportOptions,
): Promise<Blob> => {
  const imageData = await renderSelectionExport(options);
